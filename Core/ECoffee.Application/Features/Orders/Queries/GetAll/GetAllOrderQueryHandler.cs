...
namespace ECoffee.Application.Features.Orders.Queries.GetAll
{
    public class GetAllOrderQueryHandler : IRequestHandler<GetAllOrderQueryRequest, IDataResult<GetAllOrderQueryResponse>>
    {
        ...
        {
            (List<GetAllOrdersDTO> ordersDTOs, int totalCount) data = await _orderService.GetAllAsync(request.Page, request.Size);
            return new SuccessDataResult<GetAllOrderQueryResponse>("Siparişler Listelendi", new() { GetAllOrders = data.ordersDTOs, TotalCount = data.totalCount });
        }
    }
}
